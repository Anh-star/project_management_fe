export default function Badge({ type, value }) {
  ...
  const colorClass =
    colors[value] || "bg-gray-100 text-gray-600 border-gray-200";

  return (
    <span
      className={`px-2.5 py-0.5 rounded-full text-[11px] font-bold border uppercase tracking-wide ${colorClass}`}
    >
      {value?.replace("_", " ")}
    </span>
  );
