...
export default function ProjectReport({ projectId, token }) {
    ...
    useEffect(() => { fetchReport(); }, [projectId]);
                    ...
                    <table className="min-w-full text-sm text-left">
                        <thead className="bg-gray-50 text-gray-500 font-semibold uppercase text-xs tracking-wider">
                            <tr>
                                ...
                                <th className="px-6 py-4 text-center text-green-700">Hoàn thành</th>
                        ...
                        <tbody className="divide-y divide-gray-100 bg-white">
                                    ...
                                    <tr key={m.user_id} className="hover:bg-indigo-50/30 transition-colors">
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <span className="font-medium text-gray-900 block">{m.username}</span>
                                                    <span className="text-xs text-gray-400">{m.email}</span>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-6 py-4 text-center text-gray-600 font-medium bg-gray-50/50">{total}</td>
                                        <td className="px-6 py-4 text-center text-green-600 font-bold">{done}</td>
                                        <td className="px-6 py-4 text-center text-red-500 font-medium">{m.overdue_tasks}</td>
                                                ...
                                                <div className="w-full bg-gray-200 rounded-full h-2.5 overflow-hidden shadow-inner">
                                                    <div 
                                                        className={`h-full rounded-full transition-all duration-1000 shadow-sm ${
                                                            ...
                                                            percent > 75 ? 'bg-indigo-500' : 
                                                            percent > 25 ? 'bg-blue-400' : 'bg-orange-400'
                                                        }`} 
                                                        style={{ width: `${percent}%` }} 
                                                    />
                                                </div>
                            ...
                            {report?.members.length === 0 && (
                                <tr>
                                    <td colSpan="5" className="text-center py-12 text-gray-400">Chưa có thành viên nào trong dự án.</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
