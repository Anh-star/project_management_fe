...
// ... (Helper functions, TaskForm, TaskItem GIỮ NGUYÊN NHƯ CŨ) ...
...
function ProjectDetailsContent() {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            if (resAll.ok) { const all = await resAll.json(); setProject(all.find(p => p.id === parseInt(projectId))); }
            ...
            <header className={styles.header}>
                <div className={styles.headerContent}>
                    <div>
                        ...
                        <div className="flex items-center gap-4 mt-2"><button onClick={handleOpenInviteModal} className={styles.memberBtn}>👥 {members.length} Thành viên</button></div>
                    </div>
                    <div className={styles.headerActions}>
                        {/* --- 1. CHÈN CHUÔNG VÀO ĐÂY --- */}
                        <NotificationBell />
                    ...
                    <>
                        ...
                        <div className="flex justify-between items-center mb-6">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <select className="text-xs border border-gray-300 rounded px-2 py-1 outline-none focus:border-indigo-500 cursor-pointer" value={priorityFilter} onChange={(e) => setPriorityFilter(e.target.value)}><option value="">Tất cả</option><option value="URGENT">Khẩn cấp</option><option value="HIGH">Cao</option><option value="MEDIUM">Trung bình</option><option value="LOW">Thấp</option></select>
                                
                                <span className="text-xs text-gray-500 ml-2">Trạng thái:</span>
                                <select className="text-xs border border-gray-300 rounded px-2 py-1 outline-none focus:border-indigo-500 cursor-pointer" value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)}><option value="">Tất cả</option><option value="TODO">Cần làm</option><option value="IN_PROGRESS">Đang làm</option><option value="REVIEW">Review</option><option value="DONE">Hoàn thành</option></select>
                            </div>
                        </div>
                        ...
                        <div className="space-y-4">
                            {tasks.length > 0 ? tasks.map((task) => (<TaskItem key={task.id} task={task} projectId={projectId} token={token} onRefresh={fetchData} members={members} currentUser={user} onRequestDelete={openConfirmDelete} />)) : !isCreating && <div className={styles.emptyState}><p className="text-gray-500">Không tìm thấy công việc nào.</p></div>}
                        </div>
                    </>
                ) : (
...
export default function Page() {
    return (<Suspense fallback={<div className="p-8 text-center">Đang tải...</div>}><ProjectDetailsContent /></Suspense>);
}